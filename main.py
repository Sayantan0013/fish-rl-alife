...
- Check how the observation is constructed
...
env.select_fish_types(4,0,0)
env.select_shark_types(4)

print(env.sharks)

env = MultiAgentEnvAECWrapper(env)


model = PPO('MlpPolicy',env,verbose=1,tensorboard_log=env.log_dir)
# model = PPO.load("ppo_torus_sharks_with_penalty",env=env)

model.learn(600_000)
model.save('ppo_torus_AEC')
...
for k in range(10):
    ...
    while not env.env.is_finished:
        i += 1
        action, _ = model.predict(obs,deterministic=True)
        # action = model.action_space.sample()
        obs, reward, done, trunc , _ = env.step(action, 'm1')
        time.sleep(0.01)
